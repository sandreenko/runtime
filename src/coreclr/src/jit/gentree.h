...
private:
    ...
    unsigned m_stackByteSize; // The number of bytes that this argument is occupying on the stack with padding.
...
public:
    ...
    GenTreePutArgStk(genTreeOps   oper, var_types type, GenTree* op1, unsigned stackByteOffset
        ...
        PUT_STRUCT_ARG_STK_ONLY_ARG(m_stackByteSize(stackByteSize))
...
#endif
    {
        DEBUG_ARG_SLOTS_ASSERT(stackByteOffset == slotNum * TARGET_POINTER_SIZE);
#if defined(FEATURE_PUT_STRUCT_ARG_STK)
        DEBUG_ARG_SLOTS_ASSERT(m_stackByteSize  == gtNumSlots * TARGET_POINTER_SIZE);
#endif
    }
