...
private:
    ...
    void MorphStructField(GenTree* node, GenTree* user)
    {
        ...
        noway_assert(node->OperGet() == GT_FIELD);
        ...
        noway_assert((node->gtFlags & GTF_GLOB_REF) || ((obj != nullptr) && (obj->gtOper == GT_LCL_VAR)));
        ...
        if ((obj != nullptr) && (obj->gtOper == GT_LCL_VAR))
        {
            ...
            if (varTypeIsStruct(obj))
            {
                if (varDsc->lvPromoted)
                {
...
#ifdef DEBUG
                        else if (node->TypeGet() == TYP_STRUCT)
                        {
                    ...
                    node->AsLclVarCommon()->SetLclNum(fieldLclIndex);
                    node->gtType = fieldType;
                    node->gtFlags &= GTF_NODE_MASK; // Note: that clears all flags except `GTF_COLON_COND`.

                    if (user->gtOper == GT_ASG)
                    {
                        if (user->AsOp()->gtOp1 == node)
                        {
                        ...
                        if (varTypeIsStruct(user) && user->AsOp()->gtOp2 == node && !varTypeIsStruct(node))
                        {
                            node->gtFlags |= GTF_DONT_CSE;
                        }
                    }
            ...
            else
            {
                ...
                if (node->TypeGet() == obj->TypeGet())
                {
                    ...
                    if ((user->gtOper == GT_ASG) && (user->AsOp()->gtOp1 == node))
                    {
                        node->gtFlags |= GTF_VAR_DEF;
                        node->gtFlags |= GTF_DONT_CSE;
                    }

                    JITDUMP("Replacing the field in normed struct with local var V%02u\n", lclNum);
                }
            }
        }
